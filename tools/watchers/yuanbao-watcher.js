/**
 * Yuanbao Real-Time DOM Watcher
 *
 * Usage:
 * 1) Open https://yuanbao.tencent.com/chat/
 * 2) Paste this script into DevTools Console BEFORE sending a prompt
 * 3) Send a prompt and wait until response finishes
 * 4) Copy logs marked with "CANDIDATE" and "STATE"
 */

(() => {
  const LOG = "[YuanbaoWatcher]";
  const seen = new Set();
  ...
  const TEXT_HINTS = ["停止生成", "停止回答", "停止", "思考中", "正在思考", "发送", "生成中"];
  ...
  function logCandidate(el, reason) {
    ...
    console.log(
      `${LOG} CANDIDATE (${reason}) <${tag}> class="${cls.slice(0, 120)}" aria="${aria}" title="${title}" text="${text}"`
    );
    if (selectors.length) {
  ...
  function scanNow(reason) {
    const all = document.querySelectorAll("button, [role='button'], svg, span, i, div");
    for (const el of all) {
  ...
  scanNow("initial");
  console.log(
    `${LOG} Started. Send a message now, wait until completion, then run: window.__yuanbaoWatcherStop()`
  );

  window.__yuanbaoWatcherStop = () => {
    observer.disconnect();
  ...
  setTimeout(() => {
    if (window.__yuanbaoWatcherStop) window.__yuanbaoWatcherStop();
  }, 90000);
})();
